...
def parse_invoice_table_text(text: str) -> list:
    ...
    while i < len(table_lines):
        ...
        while j < len(table_lines) and (j - i) <= 10:
            ...
            elif re.fullmatch(r"\d{1,2}", nxt) and qty is not None:  pass
            j += 1
...
def parse_invoice(text: str, line_items=None):
    ...
    # FIX: require letter to avoid matching address numbers like "No. 7"
    invoice_number = find(
        r"Invoice\s*(?:Number|No|#|Num)[\.:\s/]*"
        r"([A-Za-z][A-Za-z0-9\-/]+|[A-Za-z0-9]{3,}[/\-][A-Za-z0-9\-/]+)"
    )
    ...
    for vi, vline in enumerate(lines_v):
        if re.search(r"bill\s*to", vline, re.IGNORECASE):
            for nxt in lines_v[vi+1:vi+8]:
                if LABEL_PAT.search(nxt): continue
                if re.fullmatch(r"[A-Za-z0-9/\-]+", nxt) and "/" in nxt: continue
